...
public class PizzaPedidaController {
    ...
    public PizzaPedida findById(@PathVariable long id){
        ...
        if (resultado.isEmpty()){
            throw new RuntimeException("Pizza pedida não encontrada!");
        }else{
            return resultado.get();
        }
    }

    @RequestMapping(value = "/editarpizzapedida/{id}", method = RequestMethod.PUT)
    ...
    @RequestMapping(value = "/excluirpizzapedida/{id}", method = RequestMethod.DELETE)
